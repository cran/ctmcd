...
function(x,
                ...
                eps = 1e-4,
                expmethod = "PadeRBS") {
  ### Implementation according to Oakes
  ### Direct Calculation of the Information Matrix via the EM Algorithm
  ### Journal of the Royal Statistical Society B, 1999
  ### and Bladt and Sorensen
  ### Efficient Estimation of Transition Rates Between Credit Ratings From
  ...
  ### Functions for Expected Number of State Transitions and Holding Times
  ERiT_func = function(x, Q, i0, j0, i, j, ...) {
    ...
    augmat = rbind(cbind(Q, uitui), cbind(matrix(0, n, n), Q))
    
    caseRiT = expm(te * augmat, method = expmethod)[1:n, (n + 1):(2 * n)]
  ...
  ENijT_func = function(x, Q, i0, j0, i, j, ...) {
    ...
    augmat = rbind(cbind(Q, uituj), cbind(matrix(0, n, n), Q))
    caseNijT = Q[i, j] * expm(te * augmat, method = expmethod)[1:n, (n + 1):(2 *
                                                                               n)]
  ...
  ### Parameter Reduction: Neglect of Generator Matrix Elements < eps
  ParMat = x$par
  ...
  for (k in 1:length(ivec)) {
    for (l in 1:length(ivec)) {
      if (l == k) {
        ...
        FI[k, k] = 1 / x$par[ivec[k], jvec[k]] ^ 2 * x$ENijT[ivec[k], jvec[k]] -
          1 / x$par[ivec[k], jvec[k]] * ENijTder + ERiTder
        
      } else{
        ERiTder = grad(
          ERiT_func,
          x = x$par[ivec[l], jvec[l]],
          Q = x$par,
          i0 = ivec[l],
          j0 = jvec[l],
          i = ivec[k],
        ...
        FI[k, l] = -1 / x$par[ivec[k], jvec[k]] * ENijTder + ERiTder
      }
    }
  }
  ...
  for (i in unique(ivec)) {
    ...
    if (length(elem) == 1) {
      ...
      for (k in 1:ncol(combs)) {
        par1 = intersect(which(ivec == i), which(jvec == combs[1, k]))
        par2 = intersect(which(ivec == i), which(jvec == combs[2, k]))
  ...
  ### Calculate Boundaries of Wald Confidence Interval
